...
static int promptpass(char *user, char *passwd)
{
    ...
    if (pipe(p)) {
	warnlog("Can't make a pipe for %s", promptprog);
	return 0;
    }
    if ((kid = fork()) == (pid_t) -1) {
	warnlog("Can't fork to run %s", promptprog);
	close(p[0]);
	close(p[1]);
	return 0;
    }
	...
	if (red < 0) {
	    if (errno == EINTR)
		continue;
	    errorlog("Can't read secret from %s: %m", promptprog);
	    readgood = -1;
	    break;
	}
	readgood += red;
	...
	if (errno != EINTR) {
	    warnlog("error waiting for %s: %m", promptprog);
	    break;
	}
    }
	...
	return 0;
    if (!WIFEXITED(wstat))
	warnlog("%s terminated abnormally", promptprog);
    if (WEXITSTATUS(wstat))
	warnlog("%s exited with code %d", promptprog, WEXITSTATUS(status));
