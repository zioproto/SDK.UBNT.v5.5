...
getHWaddr(int sock, char const *ifname, unsigned char *hwaddr)
{
	...
	if (ifr->ifr_addr.sa_family == AF_LINK) {
		...
		if (found) {
		    fatallog("interface %s has more than one ethernet address", ifname);
		} else {
		    found = 1;
    ...
    if (!found) {
        fatallog("interface %s has no ethernet address", ifname);
    }
}
...
sendPacket(PPPoEConnection *conn, int sock, PPPoEPacket *pkt, int size)
{
	...
	fatallog("sendPacket: write: %m");
    }
#elif defined(HAVE_STRUCT_SOCKADDR_LL)
    if (send(sock, pkt, size, 0) < 0) {
	fatallog("sendPacket: send: %m");
	return -1;
    }
#else
#ifdef USE_DLPI
...
#define ABS(x)          ((x) < 0 ? -(x) : (x))
    ...
    if (!conn) {
	fatallog("relay and server not supported on Linux 2.0 kernels");
    }
    strcpy(sa.sa_data, conn->ifName);
    if (sendto(sock, pkt, size, 0, &sa, sizeof(sa)) < 0) {
	fatallog("sendPacket: sendto: %m");
    }
#endif
#endif
    return 0;
...
void dlunitdatareq(int fd, u_char *addrp, int addrlen, u_long minpri, u_long maxpri, u_char *datap, int datalen)
{
        ...
        if (putmsg(fd, &ctl, &data, 0) < 0)
                fatallog("dlunitdatareq: putmsg: %m");
}
...
void dlinfoack(int fd, char *bufp)
{
		...
		fatallog("dlinfoack: response ctl.len too short: %d", ctl.len); 
	}

        if (flags != RS_HIPRI)
                fatallog("dlinfoack: DL_INFO_ACK was not M_PCPROTO");

        if (ctl.len < sizeof (dl_info_ack_t)) {
		fatallog("dlinfoack: short response ctl.len: %d", ctl.len); 
	}
}
...
void dlbindack(int fd, char *bufp)
{
        ...
        if (flags != RS_HIPRI)
                fatallog("dlbindack: DL_OK_ACK was not M_PCPROTO");

        if (ctl.len < sizeof (dl_bind_ack_t)) {
		fatallog("dlbindack: short response ctl.len: %d", ctl.len);
	}
}
...
void strgetmsg(int fd, struct strbuf *ctlp, struct strbuf *datap, int *flagsp, char *caller)
{
        ...
        *flagsp = 0;
		...
		fatallog("%s: MORECTL|MOREDATA", caller);
	}
                
        if (rc & MORECTL) {
		fatallog("%s: MORECTL", caller);
	}
        
        if (rc & MOREDATA) {
		fatallog("%s: MOREDATA", caller);
	}
		...
		fatallog("getmsg: control portion length < sizeof (long): %d", ctlp->len);
	}
}

void sigalrm(int sig)
{
        fatallog("sigalrm: TIMEOUT");
}

void expecting(int prim, union DL_primitives *dlp)
{
        if (dlp->dl_primitive != (u_long)prim) {
		fatallog("expected %s got %s", dlprim(prim), dlprim(dlp->dl_primitive));
	}
}
