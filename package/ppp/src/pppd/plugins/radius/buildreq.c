...
unsigned char rc_get_seqnbr(void)
{
	...
	{
		errorlog("rc_get_seqnbr: couldn't open sequence file %s: %s", seqfile, strerror(errno));
	...
	while (do_lock_exclusive(fileno(sf))!= 0)
	{
		if (errno != EWOULDBLOCK) {
			errorlog("rc_get_seqnbr: flock failure: %s: %s", seqfile, strerror(errno));
			fclose(sf);
			return rc_guess_seqnbr();
		}
		tries++;
	...
	if (tries > 10) {
		errorlog("rc_get_seqnbr: couldn't get lock after %d tries: %s", tries-1, seqfile);
		fclose(sf);
		return rc_guess_seqnbr();
	}
	...
	if (fscanf(sf, "%d", &seq_nbr) != 1) {
		if (pos != ftell(sf)) {
			/* file was not empty */
			errorlog("rc_get_seqnbr: fscanf failure: %s", seqfile);
		}
	...
	if (do_unlock(fileno(sf)) != 0)
		errorlog("rc_get_seqnbr: couldn't release lock on %s: %s", seqfile, strerror(errno));
