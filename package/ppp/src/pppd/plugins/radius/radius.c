...
plugin_init(void)
{
    ...
    infolog("RADIUS plugin initialized.");
}
		   ...
		   char *message, int message_space)
{
	...
	errorlog("RADIUS: Challenge type %u unsupported", digest->code);
	return 0;
    }
		   ...
		   unsigned char *challenge)
{
    ...
    if (vp->lvalue != 32) {
	errorlog("RADIUS: Incorrect attribute length (%d) for MS-CHAP-MPPE-Keys",
	      vp->lvalue);
	return -1;
    }
...
radius_setmppekeys2(VALUE_PAIR *vp, REQUEST_INFO *req_info)
{
    ...
    if (vp->lvalue != 34) {
	errorlog("RADIUS: Incorrect attribute length (%d) for MS-MPPE-%s-Key",
	      vp->lvalue, type);
	return -1;
    }

    if ((salt[0] & 0x80) == 0) {
	errorlog("RADIUS: Illegal salt value for MS-MPPE-%s-Key attribute", type);
	return -1;
    }
	...
	errorlog("RADIUS: Incorrect key length (%d) for MS-MPPE-%s-Key attribute",
	      (int) plain[0], type);
	return -1;
    }
...
radius_acct_start(void)
{
	...
	warnlog("Accounting START failed for %s", rstate.user);
    } else {
...
radius_acct_stop(void)
{
    ...
    if (result != OK_RC) {
	/* RADIUS server could be down so make this a warning */
	warnlog("Accounting STOP failed for %s", rstate.user);
    }
    rc_avpair_free(send);
}
...
radius_acct_interim(void *ignored)
{
    ...
    if (result != OK_RC) {
	/* RADIUS server could be down so make this a warning */
	warnlog("Interim accounting failed for %s", rstate.user);
    }
    rc_avpair_free(send);
