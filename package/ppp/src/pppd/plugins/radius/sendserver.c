...
int rc_send_server (SEND_DATA *data, char *msg, REQUEST_INFO *info)
{
	...
	if (sockfd < 0)
	{
		...
		errorlog("rc_send_server: socket: %s", strerror(errno));
		return (ERROR_RC);
	}
		   ...
		   getsockname (sockfd, (struct sockaddr *) sin, &length) < 0)
	{
		close (sockfd);
		memset (secret, '\0', sizeof (secret));
		errorlog("rc_send_server: bind: %s: %m", server_name);
		return (ERROR_RC);
	}
	...
	for (;;)
	{
		...
		if (select (sockfd + 1, &readfds, NULL, NULL, &authtime) < 0)
		{
			if (errno == EINTR)
				continue;
			errorlog("rc_send_server: select: %m");
			memset (secret, '\0', sizeof (secret));
			close (sockfd);
			return (ERROR_RC);
		}
		...
		{
			errorlog("rc_send_server: no reply from RADIUS server %s:%u",
	...
	if (length <= 0)
	{
		errorlog("rc_send_server: recvfrom: %s:%d: %m", server_name,\
		      data->svc_port);
		close (sockfd);
		memset (secret, '\0', sizeof (secret));
		return (ERROR_RC);
			   ...
			   unsigned char *vector, unsigned char seq_nbr)
{
	...
	{
		errorlog("rc_check_reply: received RADIUS server response with invalid length");
		return (BADRESP_RC);
	}
	...
	{
		errorlog("rc_check_reply: not enough buffer space to verify RADIUS server response");
		return (BADRESP_RC);
	}
	/* Verify that id (seq. number) matches what we sent */
	if (auth->id != seq_nbr)
	{
		errorlog("rc_check_reply: received non-matching id in RADIUS server response");
		return (BADRESP_RC);
	}
		    ...
		    AUTH_VECTOR_LEN) != 0)
	{
		...
		if (auth->code == PW_ACCOUNTING_RESPONSE)
			return (OK_RC);
#endif
		errorlog("rc_check_reply: received invalid reply digest from RADIUS server");
		return (BADRESP_RC);
	}
