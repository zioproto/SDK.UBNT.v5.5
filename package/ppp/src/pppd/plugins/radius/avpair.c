	...
	switch (vp->type)
	{
		case PW_TYPE_STRING:
			...
			if (((len == 0) && (strlen ((char *) pval)) > AUTH_STRING_LEN)
			    || (len > AUTH_STRING_LEN)) {
				errorlog("rc_avpair_assign: bad attribute length");
				return result;
		    }
		...
		default:
			errorlog("rc_avpair_assign: unknown attribute %d", vp->type);
	}
	return result;
}
...
VALUE_PAIR *rc_avpair_new (int attrid, void *pval, int len, int vendorcode)
{
	...
	{
		errorlog("rc_avpair_new: unknown attribute %d", attrid);
	}
	else
	{
...
VALUE_PAIR *rc_avpair_gen (AUTH_HDR *auth)
{
	...
	while (length > 0)
	{
		...
		if (attrlen < 0)
		{
			errorlog("rc_avpair_gen: received attribute with invalid length");
			break;
		}
		...
		if ((attr = rc_dict_getattr (attribute, VENDOR_NONE)) == (DICT_ATTR *) NULL)
		{
			*buffer= '\0';	/* Initial length. */
			...
			warnlog("rc_avpair_gen: received unknown attribute %d of length %d: 0x%s",
		...
		else
		{
			...
			switch (attr->type)
			{
			    ...
			    default:
				warnlog("rc_avpair_gen: %s has unknown type", attr->name);
				free (pair);
				break;
			}
...
int rc_avpair_parse (char *buffer, VALUE_PAIR **first_pair)
{
	...
	while (*buffer != '\n' && *buffer != '\0')
	{
		...
		switch (mode)
		{
			...
			{
				errorlog("rc_avpair_parse: unknown attribute");
				if (*first_pair) {
			...
			{
				errorlog("rc_avpair_parse: missing or misplaced equal sign");
				if (*first_pair) {
					rc_avpair_free(*first_pair);
					*first_pair = (VALUE_PAIR *) NULL;
				}
			...
			switch (pair->type)
			{
				...
				else
				{
					if ((dval = rc_dict_findval (valstr))
							== (DICT_VALUE *) NULL)
					{
						errorlog("rc_avpair_parse: unknown attribute value: %s", valstr);
						if (*first_pair) {
							rc_avpair_free(*first_pair);
							*first_pair = (VALUE_PAIR *) NULL;
						}
			    ...
			    default:
				errorlog("rc_avpair_parse: unknown attribute type %d", pair->type);
				if (*first_pair) {
					rc_avpair_free(*first_pair);
					*first_pair = (VALUE_PAIR *) NULL;
				}
...
int rc_avpair_tostr (VALUE_PAIR *pair, char *name, int ln, char *value, int lv)
{
	...
	*name = *value = '\0';
	...
	if (!pair || pair->name[0] == '\0') {
		errorlog("rc_avpair_tostr: pair is NULL or empty");
		return (-1);
	}
	...
	switch (pair->type)
	{
	    ...
	    default:
		errorlog("rc_avpair_tostr: unknown attribute type %d", pair->type);
		return (-1);
		break;
	}
...
VALUE_PAIR *rc_avpair_readin(FILE *input)
{
	...
	while (fgets(buffer, sizeof(buffer), input) != NULL)
	{
		...
		if (rc_avpair_parse(q, &vp) < 0) {
			errorlog("rc_avpair_readin: malformed attribute: %s", buffer);
			rc_avpair_free(vp);
			return NULL;
		}
	}
