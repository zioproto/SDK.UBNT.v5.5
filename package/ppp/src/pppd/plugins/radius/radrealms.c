	     ...
	     SERVER **acctserver)
{
    ...
    if (realm) {
	infolog("Looking up servers for realm '%s'", realm);
    } else {
	infolog("Looking up servers for DEFAULT realm");
    }
    if (realm) {
	if (*(++realm) == '\0') {
	    realm = NULL;
    ...
    infolog("Reading %s", radrealms_config);
	...
	if ((realm != NULL && strcmp(p, realm) == 0) ||
	    ...
	    s->port[s->max] = atoi(p);
	    infolog(" - Port is '%d'", s->port[s->max]);
	    s->max++;
	} else 
	    infolog(" - Skipping realm '%s'", p);
    }
    fclose(fd);
...
plugin_init(void)
{
    ...
    add_options(Options);
    infolog("RADIUS Realms plugin initialized.");
}
