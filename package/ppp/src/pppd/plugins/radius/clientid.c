...
int rc_read_mapfile(char *filename)
{
	...
	{
		errorlog("rc_read_mapfile: can't read %s: %s", filename, strerror(errno));
		return (-1);
	}
	...
	while (fgets(buffer, sizeof(buffer), mapfd) != NULL)
	{
		...
		if (( c = strchr(q, ' ')) || (c = strchr(q,'\t'))) {
			...
			*c = '\0'; c++;
			...
			errorlog("rc_read_mapfile: malformed line in %s, line %d", filename, lnr);
			return (-1);
...
UINT4 rc_map2id(char *name)
{
	...
	*ttyname = '\0';
	...
	warnlog("rc_map2id: can't find tty %s in map database", ttyname);
