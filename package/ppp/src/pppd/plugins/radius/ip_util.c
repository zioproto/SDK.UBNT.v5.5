...
UINT4 rc_get_ipaddr (char *host)
{
	...
	{
		errorlog("rc_get_ipaddr: couldn't resolve hostname: %s", host);
		return ((UINT4) 0);
	}
...
const char *rc_ip_hostname (UINT4 h_ipaddr)
{
	...
	if ((hp = gethostbyaddr ((char *) &n_ipaddr, sizeof (struct in_addr),
			    AF_INET)) == NULL) {
		errorlog("rc_ip_hostname: couldn't look up host by addr: %08lX", h_ipaddr);
	}
...
UINT4 rc_own_ipaddress(void)
{
	...
	if (!this_host_ipaddr) {
		if ((this_host_ipaddr = rc_get_ipaddr (hostname)) == 0) {
			errorlog("rc_own_ipaddress: couldn't get own IP address");
			return 0;
		}
	}
