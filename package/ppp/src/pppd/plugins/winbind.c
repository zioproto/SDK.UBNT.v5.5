...
plugin_init(void)
{
    ...
    infolog("WINBIND plugin initialized.");
}
			   ...
			   char **error_string) 
{
	...
	if (ntlm_auth == NULL)
        ...
        /* Make first child */
        if (pipe(child_out) == -1) {
                errorlog("pipe creation failed for child OUT!");
		return NOT_AUTHENTICATED;
        }

        if (pipe(child_in) == -1) {
                errorlog("pipe creation failed for child IN!");
		return NOT_AUTHENTICATED;
        }
	...
	if (forkret == 0) {
		...
		if (setuid(uid) == -1 || getuid() != uid)
			fatallog("pppd/winbind: could not setuid to %d: %m", uid);
		execl("/bin/sh", "sh", "-c", ntlm_auth, NULL);  
		fatallog("pppd/winbind: could not exec /bin/sh: %m");
	}
	...
	while (fgets(buffer, sizeof(buffer)-1, pipe_out) != NULL) {
		...
		if (strcmp(message, ".") == 0) {
			...
			if (strcasecmp(parameter, "Yes") == 0) {
				...
				noticelog("Winbind has declined authentication for user!");
			...
			if (nt_key) { 
				if (strhex_to_str(nt_key, 32, parameter) == 16) {
					...
					noticelog("NT session key for user was not 16 bytes!");
				}
			}
			...
			noticelog("unrecognised input from ntlm_auth helper - %s: %s", message, parameter); 
		}
	}

        /* parent */
        if (close(child_out[0]) == -1) {
                noticelog("error closing pipe?!? for child OUT[0]");
                return NOT_AUTHENTICATED;
        }

       /* parent */
        if (close(child_in[1]) == -1) {
                noticelog("error closing pipe?!? for child IN[1]");
                return NOT_AUTHENTICATED;
        }
	...
	if ((authenticated == AUTHENTICATED) && nt_key && !got_user_session_key) {
		noticelog("Did not get user session key, despite being authenticated!");
		return NOT_AUTHENTICATED;
	}
		    ...
		    char *message, int message_space)
{
	...
	case CHAP_MICROSOFT:
	{
		...
		if (response[MS_CHAP_USENT]) {
			...
			noticelog("Peer request for LANMAN auth not supported");
			return NOT_AUTHENTICATED;
#endif /* MSLANMAN */
		}
	...
	default:
		errorlog("WINBIND: Challenge type %u unsupported", digest->code);
	}
	return NOT_AUTHENTICATED;
}
