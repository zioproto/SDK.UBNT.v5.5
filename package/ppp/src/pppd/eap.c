...
void *arg;
{
	...
	errorlog("EAP: timeout waiting for Request from peer");
...
eap_state *esp;
{
	...
	if (esp->es_server.ea_maxrequests > 0 &&
		...
		if (esp->es_server.ea_responses > 0)
			errorlog("EAP: too many Requests sent");
		else
			errorlog("EAP: no response to Requests");
		eap_send_failure(esp);
		return;
	}
...
int unit;
{
	...
	if (eap_client_active(esp)) {
		errorlog("EAP authentication failed due to Protocol-Reject");
		auth_withpeer_fail(unit, PPP_EAP);
	}
	if (eap_server_active(esp)) {
		errorlog("EAP authentication of peer failed on Protocol-Reject");
...
int len;
{
	...
	if (esp->es_client.ea_maxrequests != 0 &&
		...
		infolog("EAP: received too many Request messages");
		if (esp->es_client.ea_timeout > 0) {
			UNTIMEOUT(eap_client_timeout, (void *)esp);
		}
		auth_withpeer_fail(esp->es_unit, PPP_EAP);
	...
	if (len <= 0) {
		errorlog("EAP: empty Request message discarded");
		return;
	}
	...
	case EAPT_IDENTITY:
		if (len > 0)
			infolog("EAP: Identity prompt \"%.*q\"", len, inp);
#ifdef USE_SRP
		if (esp->es_usepseudo &&
		    (esp->es_usedpseudo == 0 ||
			(esp->es_usedpseudo == 1 &&
	...
	case EAPT_NOTIFICATION:
		if (len > 0)
			infolog("EAP: Notification \"%.*q\"", len, inp);
	...
	case EAPT_NAK:
		 ...
		 */
		warnlog("EAP: unexpected Nak in Request; ignored");
		/* Return because we're waiting for something real. */
		return;

	case EAPT_MD5CHAP:
		if (len < 1) {
			errorlog("EAP: received MD5-Challenge with no data");
		...
		if (vallen < 8 || vallen > len) {
			errorlog("EAP: MD5-Challenge with bad length %d (8..%d)",
			    vallen, len);
	...
	case EAPT_SRP:
		if (len < 1) {
			errorlog("EAP: received empty SRP Request");
			/* Bogus request; wait for something real. */
			return;
		}
		...
		case EAPSRP_CHALLENGE:
			...
			if (tc == NULL) {
				...
				len--;
				if (vallen >= len) {
					errorlog("EAP: badly-formed SRP Challenge"
					    " (name)");
		...
		case EAPSRP_SKEY:
			...
			if (tc == NULL) {
				warnlog("EAP: peer sent Subtype 2 without 1");
				eap_send_nak(esp, id, EAPT_MD5CHAP);
				break;
			}
			if (esp->es_client.ea_skey != NULL) {
				 ...
				 */
				if (id != esp->es_client.ea_id) {
					warnlog("EAP: ID changed from %d to %d "
				...
				if (esp->es_client.ea_skey == NULL) {
					...
					errorlog("EAP: SRP server is rogue");
					goto client_failure;
				}
			}
		...
		case EAPSRP_SVALIDATOR:
			...
			if (tc == NULL || esp->es_client.ea_skey == NULL) {
				warnlog("EAP: peer sent Subtype 3 without 1/2");
				eap_send_nak(esp, id, EAPT_MD5CHAP);
				break;
			}
			/*
			...
			if (esp->es_client.ea_state == eapOpen) {
				...
				if (len < 0 || t_clientverify(tc, inp +
					sizeof (u_int32_t)) != 0) {
					errorlog("EAP: SRP server verification "
					    "failed");
					goto client_failure;
				}
		...
		case EAPSRP_LWRECHALLENGE:
			if (len < 4) {
				warnlog("EAP: malformed Lightweight rechallenge");
				return;
			}
			SHA1Init(&ctxt);
			vals[0] = id;
		...
		default:
			errorlog("EAP: unknown SRP Subtype %d", vallen);
			eap_send_nak(esp, id, EAPT_MD5CHAP);
			break;
		}
		break;
	...
	default:
		infolog("EAP: unknown authentication type %d; Naking", typenum);
		eap_send_nak(esp, id, EAPT_SRP);
		break;
	}
...
int len;
{
	...
	if (len <= 0) {
		errorlog("EAP: empty Response message discarded");
		return;
	}
	...
	case EAPT_IDENTITY:
		...
		infolog("EAP: unauthenticated peer name \"%.*q\"", len, inp);
		if (esp->es_server.ea_peer != NULL &&
		    esp->es_server.ea_peer != remote_name)
			free(esp->es_server.ea_peer);
	...
	case EAPT_NAK:
		if (len < 1) {
			infolog("EAP: Nak Response with no suggested protocol");
			eap_figure_next_state(esp, 1);
			break;
		}
	...
	case EAPT_MD5CHAP:
		if (esp->es_server.ea_state != eapMD5Chall) {
			errorlog("EAP: unexpected MD5-Response");
			eap_figure_next_state(esp, 1);
			break;
		}
		if (len < 1) {
			errorlog("EAP: received MD5-Response with no data");
			eap_figure_next_state(esp, 1);
			break;
		}
		GETCHAR(vallen, inp);
		...
		if (vallen != 16 || vallen > len) {
			errorlog("EAP: MD5-Response with bad length %d", vallen);
			eap_figure_next_state(esp, 1);
			break;
		}
	...
	case EAPT_SRP:
		if (len < 1) {
			errorlog("EAP: empty SRP Response");
			eap_figure_next_state(esp, 1);
			break;
		}
		GETCHAR(typenum, inp);
		...
		case EAPSRP_CKEY:
			if (esp->es_server.ea_state != eapSRP1) {
				errorlog("EAP: unexpected SRP Subtype 1 Response");
				eap_figure_next_state(esp, 1);
				break;
			}
			A.data = inp;
			...
			if (esp->es_server.ea_skey == NULL) {
				...
				errorlog("EAP: bogus A value from client");
				eap_send_failure(esp);
			} else {
				eap_figure_next_state(esp, 0);
			}
		...
		case EAPSRP_CVALIDATOR:
			if (esp->es_server.ea_state != eapSRP2) {
				errorlog("EAP: unexpected SRP Subtype 2 Response");
				eap_figure_next_state(esp, 1);
				break;
			}
			if (len < sizeof (u_int32_t) + SHA_DIGESTSIZE) {
				errorlog("EAP: M1 length %d < %d", len,
			...
			if (t_serververify(ts, inp)) {
				infolog("EAP: unable to validate client identity");
				eap_send_failure(esp);
				break;
			}
			eap_figure_next_state(esp, 0);
		...
		case EAPSRP_ACK:
			if (esp->es_server.ea_state != eapSRP3) {
				errorlog("EAP: unexpected SRP Subtype 3 Response");
				eap_send_failure(esp);
				break;
			}
			esp->es_server.ea_type = EAPT_SRP;
		...
		case EAPSRP_LWRECHALLENGE:
			if (esp->es_server.ea_state != eapSRP4) {
				infolog("EAP: unexpected SRP Subtype 4 Response");
				return;
			}
			if (len != SHA_DIGESTSIZE) {
				errorlog("EAP: bad Lightweight rechallenge "
				    "response");
				return;
			}
			SHA1Init(&ctxt);
			...
			if (BCMP(dig, inp, SHA_DIGESTSIZE) != 0) {
				errorlog("EAP: failed Lightweight rechallenge");
				eap_send_failure(esp);
				break;
			}
			esp->es_server.ea_state = eapOpen;
	...
	default:
		/* This can't happen. */
		errorlog("EAP: unknown Response type %d; ignored", typenum);
		return;
	}
...
int len;
{
	...
	errorlog("EAP: peer reports authentication failure");
	auth_withpeer_fail(esp->es_unit, PPP_EAP);
}
...
int inlen;
{
	...
	if (inlen < EAP_HEADERLEN) {
		errorlog("EAP: packet too short: %d < %d", inlen, EAP_HEADERLEN);
		return;
	}
	GETCHAR(code, inp);
	GETCHAR(id, inp);
	...
	if (len < EAP_HEADERLEN || len > inlen) {
		errorlog("EAP: packet has illegal length field %d (%d..%d)", len,
		    EAP_HEADERLEN, inlen);
		return;
	}
	len -= EAP_HEADERLEN;
	...
	default:				/* XXX Need code reject */
		...
		warnlog("EAP: unknown code %d received", code);
		break;
	}
}
